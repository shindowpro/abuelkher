...
add_filter( 'woocommerce_add_to_cart_redirect','dimita_quick_buy_redirect');
add_filter( 'woosw_button_position_archive', '__return_false' );
add_filter( 'woosw_button_position_single', '__return_false' );
add_filter( 'woosc_button_position_single', '__return_false' );
add_filter( 'woosc_button_position_archive', '__return_false' );
function dimita_quick_buy_redirect( $url_redirect ) {
...
function dimita_button_product(){
	...
	//Whishlist
	if(isset($dimita_settings['product-wishlist']) && $dimita_settings['product-wishlist'] && class_exists( 'WPCleverWoosw' ) ){
		add_action('woocommerce_after_shop_loop_item', 'dimita_add_loop_wishlist_link', 20 );	
	}
	//Compare
	if(isset($dimita_settings['product-compare']) && $dimita_settings['product-compare'] && class_exists( 'WPCleverWoosc' ) ){
...
function dimita_woocommerce_single_product_summary(){
	...
	add_action( 'woocommerce_after_add_to_cart_button', 'dimita_product_quick_buy_button', 10 );
}
...
function dimita_get_countdown(){
	...
	if ( $countdown_time && ( $dates >= $start_time ) ):
...
<?php }
function dimita_product_quick_buy_button() {
	$show_quick_buy = dimita_get_config('show-quick-buy',true);
	if($show_quick_buy){
		global $product;
		if ( $product->get_type() == 'external' ) {
			return;
		}
		$html = '<button class="button quick-buy">'.esc_html__("Buy Now","dimita").'</button>';
